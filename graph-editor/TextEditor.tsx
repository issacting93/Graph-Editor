...
const TextEditor = () => {
  ...
  const [notes, setNotes] = useState([
    {
      ...
      title: "Welcome Note",
      content:
        "Welcome to your minimalist editor! You can use #tags to organize your thoughts.",
    },
    {
      id: 2,
      title: "Quick Tips",
      content: "Use **bold**, *italic*, and #tags to format your notes.",
    },
  ]);
  ...
  const createNewTag = () => {
    const tagName = prompt(
      "Enter new tag name (no spaces, use - or _ for separators):",
    );
  ...
  const renderHighlightedContent = () => {
    ...
    let formattedContent = content
      ...
      .replace(
        /\`(.*?)\`/g,
        '<code class="px-1 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-sm">$1</code>',
      )
      .replace(
        /\[(.*?)\]\((.*?)\)/g,
  ...
  return (
    <div
      className={`flex h-screen transition-colors duration-300 ${darkMode ? "bg-gray-900 text-gray-100" : "bg-white text-gray-800"} ${fullscreen ? "fixed inset-0 z-50" : ""}`}
      style={{
    ...
    >
      ...
      <Drawer variant="persistent" open={showSidebar && !fullscreen}>
        <div
          className={`w-64 p-4 flex flex-col ${darkMode ? "bg-gray-800" : "bg-white"}`}
          style={{
            fontFamily:
              "Share Tech Mono, IBM Plex Mono, Courier New, monospace",
          }}
        >
      ...
      <div className="flex-grow flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-grow flex flex-col">
          ...
          {!fullscreen && tags.length > 0 && (
            <div
              className={`p-3 ${darkMode ? "bg-gray-800" : "bg-gray-100"} border-t ${darkMode ? "border-gray-700" : "border-gray-200"}`}
              style={{ fontFamily: "Barlow, sans-serif" }}
            >
          ...
          {!fullscreen && content && (
            <div
              className={`${darkMode ? "bg-gray-800 border-t border-gray-700" : "bg-gray-100 border-t border-gray-200"} p-6`}
              style={{ fontFamily: "Barlow, sans-serif" }}
            >
              <h3
                className={`text-sm font-medium mb-4 uppercase tracking-wider ${darkMode ? "text-gray-400" : "text-gray-500"}`}
              >
                Preview
              </h3>
              <div
                className={`prose max-w-none ${darkMode ? "prose-invert" : ""} prose-p:leading-relaxed prose-p:tracking-wide`}
              >
