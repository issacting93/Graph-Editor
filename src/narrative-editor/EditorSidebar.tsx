...
type NodeType = 'character' | 'location' | 'event' | 'memory' | 'item' | 
...
export const EditorSidebar: React.FC = () => {
  ...
  const NodeTemplate: React.FC<{ type: NodeType; label: string }> = ({ type, label }) => {
    const Icon = nodeIcons[type];
    return (
      <div
        draggable
      ...
      >
        ...
        <div className="flex-1">
          <div className="text-white font-medium uppercase text-sm">{label}</div>
  ...
  return (
    <div className="w-64 h-full bg-dark-900 p-4 overflow-y-auto border-r border-gray-700">
      <h2 className="text-xl font-bold mb-4 text-white uppercase tracking-wider">Node Types</h2>
