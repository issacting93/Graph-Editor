...
const mockDataSources: Record<DataCategory, DataSource[]> = {
  ...
  imported: [
    { 
      ...
      preview: 'Collection of character data with personalities, backgrounds, etc.'
    },
  ],
  generated: [
    { 
...
export const DataLibrary: React.FC = () => {
  ...
  const filteredData = mockDataSources[activeCategory].filter(item => 
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-cyber-muted p-4">
        <h2 className="text-xl font-bold text-white uppercase tracking-wider mb-4">Data Library</h2>
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className={`w-8 h-8 flex items-center justify-center rounded-full 
                    ${item.type === 'character' ? 'bg-blue-500/20' : 
                      item.type === 'location' ? 'bg-green-500/20' :
                      item.type === 'event' ? 'bg-red-500/20' : 
                      item.type === 'item' ? 'bg-yellow-500/20' : 'bg-purple-500/20'}`}
                  >
                    {item.type === 'character' ? <div className="text-blue-400">👤</div> :
                     item.type === 'location' ? <div className="text-green-400">📍</div> :
                     item.type === 'event' ? <div className="text-red-400">📅</div> :
                     item.type === 'item' ? <div className="text-yellow-400">📦</div> :
                     <div className="text-purple-400">📊</div>}
                  </div>
                  
                  <div className="ml-3 flex-1">
                    <div className="text-white text-sm font-medium">{item.name}</div>
                    <div className="text-cyber-muted text-xs">{item.description}</div>
                  </div>
                </div>

                {/* Preview tooltip */}
                {previewItem?.id === item.id && item.preview && (
                  <div className="absolute z-10 left-full top-0 ml-2 w-64 bg-cyber-dark-900 border border-cyber-blue
                    rounded-lg p-3 shadow-lg text-xs text-white">
                    <div className="font-medium mb-1 text-cyber-blue">Preview</div>
      ...
      <div className="border-t border-cyber-muted p-4">
        <div className="grid grid-cols-3 gap-2">
          <button className="bg-cyber-dark-800 hover:bg-cyber-blue/20 border border-cyber-muted
