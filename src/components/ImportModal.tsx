...
export const ImportModal: React.FC<ImportModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setImportError('Error reading file. Please make sure it\'s a valid JSON or CSV file.');
        setFileData(null);
      }
    };
  ...
  const handleImport = () => {
    try {
      ...
      setImportError('Error importing data. Please check the format and try again.');
    }
  };
  ...
  "content": "Description here"
          ...
          {activeTab === 'ai' && (
            <div>
              ...
              <div className="bg-cyber-dark-800 border border-cyber-muted rounded-lg p-4 mb-4">
                <div className="text-sm text-cyber-blue-bright font-medium mb-2">How It Works</div>
                <ul className="text-xs text-cyber-muted space-y-1 list-disc pl-5">
                  <li>Paste any narrative text, dialogue, or description</li>
                  <li>AI analyzes text to extract characters, locations, events</li>
                  <li>Automatically creates node structure with relationships</li>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button
              ...
              disabled={activeTab === 'file' && !fileData || activeTab === 'text' && !textData}
              className={`px-4 py-2 bg-cyber-blue/80 text-white rounded-lg flex items-center
                ${(activeTab === 'file' && !fileData || activeTab === 'text' && !textData)
