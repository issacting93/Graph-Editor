...
export const EnhancedGraphCanvas: React.FC = () => {
  ...
  const handleCanvasMouseDown = (e: React.MouseEvent) => {
    if (e.button === 1 || (e.button === 0 && e.altKey)) { // Middle mouse or Alt+Left click for pan
      setIsDragging(true);
      setDragStart({ x: e.clientX, y: e.clientY });
      e.preventDefault();
    } else if (e.button === 0 && !e.ctrlKey && !e.shiftKey) { // Left click with no modifiers
      ...
      >
        ...
        <svg className="absolute inset-0 pointer-events-none">
          ...
          {connections.map((connection) => {
            ...
            const c1x = sourceX + dx * 0.33 + (Math.random() - 0.5) * curveOffset;
            const c1y = sourceY + dy * 0.33 + (Math.random() - 0.5) * curveOffset;
            const c2x = sourceX + dx * 0.66 + (Math.random() - 0.5) * curveOffset;
            const c2y = sourceY + dy * 0.66 + (Math.random() - 0.5) * curveOffset;
            
            // Arrow head points
            const arrowLength = 10;
            const arrowWidth = 6;
            const angle = Math.atan2(targetY - c2y, targetX - c2x);
            const arrowPoint1X = targetX - arrowLength * Math.cos(angle) + arrowWidth * Math.sin(angle);
            const arrowPoint1Y = targetY - arrowLength * Math.sin(angle) - arrowWidth * Math.cos(angle);
            const arrowPoint2X = targetX - arrowLength * Math.cos(angle) - arrowWidth * Math.sin(angle);
            const arrowPoint2Y = targetY - arrowLength * Math.sin(angle) + arrowWidth * Math.cos(angle);
            
            return (
              <g key={connection.id} onClick={() => setSelectedConnectionId(connection.id)}>
                ...
                <polygon
                  ...
                  className={`${connection.type === 'causal' ? 'fill-cyber-blue' : 
                    connection.type === 'temporal' ? 'fill-cyber-green' : 
                    connection.type === 'conditional' ? 'fill-cyber-yellow' : 
                    'fill-cyber-pink'} 
        ...
        {showGrid && (
          <div className="absolute inset-0 pointer-events-none">
            ...
            {/* Grid points */}
            {Array.from({ length: Math.ceil(10000 / GRID_SIZE) }).map((_, i) =>
              Array.from({ length: Math.ceil(10000 / GRID_SIZE) }).map((_, j) => (
                <div
                  key={`point-${i}-${j}`}
        ...
        {nodes.map((node) => {
          ...
          const isSelected = selectedNodeId === node.id || multiSelectNodes.includes(node.id);
              ...
              onClick={(e) => {
                ...
                if (e.shiftKey) {
                  ...
                  if (multiSelectNodes.includes(node.id)) {
                    setMultiSelectNodes(prev => prev.filter(id => id !== node.id));
                  } else {
              ...
              <div className="flex items-center space-x-2 h-full">
                ...
                <div className="flex-1">
                  <div className="text-white font-medium uppercase text-sm truncate">
                    {node.attributes?.name || `${node.type.charAt(0).toUpperCase() + node.type.slice(1)}`}
                  </div>
                  {node.content && (
